...
class Buttons {
  public:
    ...
    // Returns the button associated with the analog input specified.
    ...
    static Button GetSinglePress(const Button button) {
      static Button active = Button::NONE;
      static uint32_t started_at_ms = millis();
      ...
      const uint32_t fourth_counts = millisSince(started_at_ms) / 250;
      // Auto press at a 25ms rate after holding for 5 seconds.
      if (fourth_counts >= 4 /*counts per second*/ * 5) {
        const uint32_t fortieth_counts = millisSince(started_at_ms) / 25;
    ...
    // Time when the button press started.
