...
struct PersistedSettings {
  PersistedSettings() : heat_enabled(true), cool_enabled(true), fan_always_on(false), humidity(30) {};
  uint16_t version;
...
struct Settings {
  ...
  uint32_t override_temp_started_ms;
...
static int GetSetpointTemp(const Settings& settings, const Date& date, const Mode mode) {
  ...
  // Find the setpoint time closest, but smaller than the current date.
  ...
  for (const Setpoint& setpoint : (mode == Mode::HEAT) ? settings.persisted.heat_setpoints : settings.persisted.cool_setpoints) {
    ...
    //   diff1= 19-18 = 1
    //   diff2=
    const uint16_t diff = (clock_minutes > minutes) ? clock_minutes - minutes : ((24 * 60) - minutes) + clock_minutes;
