...
TEST_F(HvacControllerThermostatTaskTest, HeatLockout) {
  settings.persisted.heat_enabled = true;
  settings.persisted.cool_enabled = true;
  ...
  // Create a cool event to test heat lockout.
  {
  ...
  }
  {
	  ...
	  new_event->start_time = clock.Millis() + 1;
  ...
  // Push the time past the cool event so heat kicks in.
  clock.Increment(Clock::MinutesToMillis(6));
  settings.now = clock.Millis();

  // We should be out of lockout now and heat.
  EXPECT_EQ(task.RunOnce(&settings), Status::kOk); 
  EXPECT_EQ(settings.hvac, HvacMode::HEAT);
}

TEST_F(HvacControllerThermostatTaskTest, CoolLockout) {
  settings.persisted.heat_enabled = true;
  settings.persisted.cool_enabled = true;
  ...
  // Create a heat event to test lockout.
  {
	  Event* new_event = &settings.events[0];
	  new_event->start_time = clock.Millis();
  ...
  // Push the time past the cool event so cool kicks in.
  clock.Increment(Clock::MinutesToMillis(6));
  settings.now = clock.Millis();
