...
TEST_F(MenusTest, EditExitsWithLeftPress) {    
  Menus menu = Menus(&settings, [](uint32_t) -> Button { return Button::LEFT; }, &clock, &display, &mock_storer);
  menu.EditSettings();
...
static Button CycleThroughEditSettings(uint32_t timeout) {
  ...
  switch (counter) {
    ...
    case 9:
      ...
      break;
    case 10:
      EXPECT_STREQ(display.GetString(string, 1,0,16), "Fan dt:060m 25% ");
      break;
    default:
      EXPECT_FALSE(true);
      return Button::NONE;       
...
TEST_F(MenusTest, CycleThroughEditSettings) {
  ...
  Menus menu = Menus(&settings, &CycleThroughEditSettings, &clock, &display, &mock_storer);
  menu.EditSettings();      
}
