...
TEST(EventsTest, SeveralEvents) {
  ...
  int i = 0;
  {
    ...
    settings.events[i].empty = false;
    settings.events[i].heat = true;
    settings.events[i].cool = false;
  ...
  uint8_t events = 0;
  EXPECT_EQ(CalculateSeconds(true, settings, &events, ms), Clock::MinutesToMillis(24+17)/1000);
  EXPECT_EQ(events, 2);
  EXPECT_EQ(CalculateSeconds(false, settings, &events, ms), Clock::MinutesToMillis(35)/1000);
  EXPECT_EQ(events, 1);
  EXPECT_NEAR(OnPercent(settings, ms), static_cast<double>(24+17)/(24+17+35), 0.00001);
