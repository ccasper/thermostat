...
class SensorStub : public Sensor {
  public:
    ...
    void EnableAsyncAssert() {
      enable_async_assert_ = true;
    };
    void SetHeaterValue(uint32_t temp) {
      heater_value_ = temp;
    };
...
class ClockStub : public Clock {
  public:
    ...
    void Increment(uint32_t millis) {
      millis_ += millis;
    }
...
class RelaysStub: public Relays {
  public:
    ...
    RelayState Get(const RelayType relay) {
      return relay_[static_cast<int>(relay)];
    };
  private:
...
class DisplayStub : public Display {
  public:
    ...
    uint8_t CompareRow(const uint8_t row, char *str_row) {
      return 0;
    };
