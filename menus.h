...
class Menus {
  public:
      ...
      while (true) {
        ...
        switch (menu_index) {
          ...
          case 2:
            ...
            break;
          case 3:
            button = SetSetpoint(0);
            break;
          case 4:
            button = SetSetpoint(1);
            break;
          case 5:
            button  = SetCoolTemp();
            break;
    ...
    Button SetSetpoint(const uint8_t setpoint) {
      uint8_t field = 0;
      int temp_x10 = settings_->persisted.setpoints[setpoint].temp_x10;
      int hour = settings_->persisted.setpoints[setpoint].hour;
      int minute = settings_->persisted.setpoints[setpoint].minute;
      uint32_t flash_ms = millis();
      bool flash_state = false;
      ...
      while (true) {
        ...
        switch (button) {
          ...
          case Button::UP:
            ...
            break;
          case Button::SELECT:
            settings_->persisted.setpoints[setpoint].temp_x10 = temp_x10;
            settings_->persisted.setpoints[setpoint].hour = hour;
            settings_->persisted.setpoints[setpoint].minute = minute;
            // Update the settings data.
            SetChangedAndPersist(settings_);
            PrintUpdatedAndWait();
            return Button::NONE;
    ...
    Button SetCoolTemp() {
      int cool_temp_x10 = settings_->persisted.cool_temp_x10;
      uint32_t flash_ms = millis();
      bool flash_state = false;
      ...
      auto update = [&]() {
        ...
        if (flash_state == true) {
          ...
          if (cool_temp_x10 / 10 < 10) {
            lcd_->print("0");
          }
          lcd_->print(cool_temp_x10 / 10);
          lcd_->print(".");
          lcd_->print(cool_temp_x10 % 10);
        }
      ...
      while (true) {
        ...
        switch (button) {
          ...
          case Button::UP:
            ...
            cool_temp_x10 += increment;
            break;
          case Button::SELECT:
            settings_->persisted.cool_temp_x10 = cool_temp_x10;
            // Update the settings data.
            SetChangedAndPersist(settings_);
            PrintUpdatedAndWait();
            return Button::NONE;
    ...
    void UpdateInformationalState() {

      if (millisSince(changed_ms_) > 2000) {
        changed_ms_ = millis();
      }
      Date date = rtc_date_->Now();
      int hours = date.hour;
    ...
    Button EditOverrideTemp() {
      ...
      while (true) {
        ...
        lcd_->setCursor(10, 1);
        int temp = GetSetpointTemp(*settings_, date);
