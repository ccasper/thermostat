...
void SetupTemperatureSensor() {
  ...
  // We only have one dallas temperature sensor on the bus. Store this to avoid needing to scan the bus on each read.
  sensor.getAddress(temperature_address, static_cast<uint8_t>(0));

  // When requesting a conversion, don't wait for the data, we'll collect the metrics later.
